...
# Function to extract the "N° OM:", "DESCRIÇÃO OM:" and "Centro de Custo" from the text
...
def extract_info(text):
    ...
    if om_match:
        try:
            ...
            # In case it's not a number, store it as is
    ...
    # Using regex to extract "Centro de Custo" and "Criticidade"
    cc_match = re.search(r'do Equipamento\s*(\S+)\s*(\S+)', text)
    if cc_match:
        try:
            info['Centro de Custo'] = int(cc_match.group(1))
        except ValueError:
            # In case it's not a number, store it as is
            info['Centro de Custo'] = cc_match.group(1)
        info['Criticidade'] = cc_match.group(2)
...
# Initialize list to collect "N° OM", "DESCRIÇÃO OM", "Centro de Custo", "Criticidade", "Data Inicio" and "Data Final" information
info_list = []
...
if info_list:
    df = pd.DataFrame(info_list, columns=[
                      'N° OM', 'Descrição OM', 'Centro de Custo', 'Criticidade', 'Data Inicio', 'Data Final'])

...
# Function to extract the "N° OM:" from the text


def extract_no_om(text):
    if "N° OM:" in text:
        start = text.find("N° OM:") + len("N° OM:")
        end = text.find("\n", start)
        if end == -1:
            end = len(text)
        no_om = text[start:end].strip()
        # Ensure the extracted "N° OM" is not too long and doesn't contain invalid characters
        # Use only the first part of the extracted text
        no_om = no_om.split()[0]
        return no_om
    return None


# Initialize list to collect "N° OM" information
no_om_list = []
...
if start_page < input_pdf_pypdf2.getNumPages():
    ...
    if no_om:
        ...
        print(
            f"Saved {no_om}.pdf from page {start_page} to {input_pdf_pypdf2.getNumPages() - 1}")
        no_om_list.append({'N° OM': no_om})

# Create a DataFrame and save to Excel
df = pd.DataFrame(no_om_list, columns=['N° OM'])

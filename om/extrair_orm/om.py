...
#     # Using regex to extract "Centro de Custo", "Criticidade", "Local de Instalação", "Descrição do Local de Instalação", "Local de Instalação Superior", and "Descrição do Local de Instalação Superior"
#     loc_match = re.search(
#         r'do Equipamento\s*(\S+)\s*(\S+)\s*(\S+)\s*(.*?)\s*[\r\n]+(\S+)\s*[\r\n]+(.*?)(?:\s*[\r\n]+|$)', text, re.DOTALL)
#     if loc_match:
#         centro_de_custo = loc_match.group(1)
#         if not centro_de_custo.isdigit() or len(centro_de_custo) not in [6, 7]:
#             info['Centro de Custo'] = loc_match.group(2)
#             info['Criticidade'] = loc_match.group(3)
#             info['Local de Instalação'] = loc_match.group(4).strip()
#             info['Descrição do Local de Instalação'] = loc_match.group(5)
#             info['Local de Instalação Superior'] = loc_match.group(6).strip()
#             info['Descrição do Local de Instalação Superior'] = ""
#         else:
#             info['Centro de Custo'] = centro_de_custo
...
#     # Move values if "Local de Instalação" is not valid
#     if not info.get('Local de Instalação').isdigit():
#         info['Local de Instalação'] = info.get(
#             'Descrição do Local de Instalação', '')
#         info['Descrição do Local de Instalação'] = info.get(
#             'Local de Instalação Superior', '')
#         info['Local de Instalação Superior'] = info.get(
...
#         'N° OM', 'Descrição OM', 'Centro de Custo', 'Criticidade',
#         'Local de Instalação', 'Descrição do Local de Instalação',
#         'Local de Instalação Superior', 'Descrição do Local de Instalação Superior',
#         'Data Inicio', 'Data Final', 'Prioridade'
#     ])
#     df.to_excel(output_excel_path, index=False)
#     print(f'Dados extraídos salvos em: {output_excel_path}')
# else:
#     print("No information extracted to save in JSON or Excel")

# print(f'PDFs separados salvos em: {output_folder}')

# # Print text from the second page of the specific PDF (for OM 202402645957)
# specific_pdf_path = os.path.join(output_folder, '202402645957.pdf')
# if os.path.exists(specific_pdf_path):
#     specific_pdf = fitz.open(specific_pdf_path)
#     if specific_pdf.page_count > 1:
#         second_page_text = specific_pdf.load_page(1).get_text("text")
#         print(
#             f"Text from the second page of 202402645957:\n{second_page_text}")
#     else:
#         print("The specific PDF does not have a second page.")
# else:
#     print("The specific PDF was not found.")
